...
仅限作者自己的电脑，可以在明确确认后初始化 CurrentUser 个人发布者：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\new-personal-signing-certificate.ps1 `
  -Attestation "I approve CurrentUser OMNIX personal publisher trust"
```

第一次运行只创建不可导出的 RSA 私钥，并把公开证书加入当前用户的 `TrustedPeople` 与 `TrustedPublisher`；它不会写入 `LocalMachine` 或 Root，也不会导出 PFX。记录输出的 40 位 `Thumbprint` 后，如果你明确理解自签名根信任的影响，再用包含实际指纹的第二份声明重新运行：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\new-personal-signing-certificate.ps1 `
  -Attestation "I approve CurrentUser OMNIX personal publisher trust" `
  -RootTrustAttestation "I approve CurrentUser Root trust for 40_HEX_CHARACTER_THUMBPRINT"
```

只有第二份声明与当前证书实际指纹完全一致时，脚本才把公开证书加入 `CurrentUser\Root`，使本机 Authenticode 验证形成有效链。它始终不写 `LocalMachine`，也不能建立公开信誉，SmartScreen 仍可能提示。
