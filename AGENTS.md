...
- For beginner-facing Agent conclusion panels, being present in the UI tree is not enough. Add stable AutomationIds, protect important placement with a static order test when possible, and capture a real screenshot showing the conclusion in the first visible working area after the relevant scan.
- For WPF GUI smokes, put AutomationIds on controls that reliably appear in UIAutomation such as buttons, text blocks, list boxes, and list items. Do not rely on decorative `Border`, `Grid`, or panel containers as the only proof target.
