...
| Source publication | Pass | Commits `46374e9` and `6053d7b` were pushed; GitHub CI runs `30333084515` and `30333783060` passed all gates. | The exact GlobalSign fallback now needs its own commit/push/CI before final packaging. |
...
| Timestamp supply chain | Pass | DigiCert failures produced no valid candidate; GlobalSign's official R45 URL was verified on an isolated copy with Valid signature/timestamp; both scripts allowlist only the exact host/path; red 2/8 then green 8/8; full 1060/1060, Release 0 errors, integrity 383/18, and parser/diff checks pass. | Timestamp-policy commit/CI and final artifact verification remain pending. |
| Timestamp resilience | Pass | Payload signer now uses three attempts at most, ten-second waits, and unchanged mandatory Authenticode/thumbprint/timestamp checks; red 1/4 then green 4/4; full 1060/1060, Release 0 errors, integrity 383/18, parser/diff checks pass. | Retry commit/CI and real final candidate remain pending. |
| Release | Warn | The real draft attempt failed closed before upload on an expected missing-tag probe; explicit exit-code handling is committed and CI-verified. | Timestamp-policy commit/CI, rebuilt assets, draft review, public Release, and latest-endpoint verification are pending. |
