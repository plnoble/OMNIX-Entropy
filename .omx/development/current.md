...
- Bootstrap outcome: the user completed the manual 0.1.2 install during this slice. A read-only uninstall-registry query confirms `OMNIX-Entropy 版本 0.1.2`, publisher `plnoble`, InstallDate `20260728` — but `InstallLocation` is `D:\Software\OMNIX-Entropy\Install\Install\`, one level deeper than the managed layout. `WindowsPersonalUpdatePathPolicy.ResolveProductRoot` therefore throws, and the in-app update path is unavailable on this installation. The installer's `DefaultDirName` is correct; `AppendDefaultDirName` defaults to yes, so confirming the pre-filled path through Inno's Browse dialog appends `Install` a second time.
- Exact next action: fix the layout defect before any release newer than 0.1.2 — make the doubled path unproducible in `installer/OMNIX-Entropy.iss`, and give the layout refusal a distinct beginner-visible message instead of the current generic preparation failure. Do not relax the layout check itself. Re-pointing the existing install requires a user-run reinstall into `D:\Software\OMNIX-Entropy\Install`.
