...
## Active Slice - 2026-07-29 Managed install-layout closure and 0.1.3 preparation
...
- Status: implementation and local gates complete. TDD red failed only the two missing behaviors; focused installer/update/release-page contracts pass 16/16; full Debug passes 1068/1068; Release builds with 0 warnings and 0 errors; source integrity reports 385 files, zero invalid UTF-8/replacement files, and 18/18 valid XAML; `git diff --check` passes. Inno explicitly sets `AppendDefaultDirName=no`; the real path policy raises a typed doubled-layout refusal; the downloader returns reinstall guidance without making an HTTP request; the release-page failure path is contract-covered. Fifteen archive EOF whitespace failures were mechanically removed, and `git diff --ignore-blank-lines --exit-code fe2e012 -- .omx/development/archive` proves no nonblank archive content changed.
- Exact next action: commit the reviewed source and records on top of `fe2e012`, push `main`, wait for green GitHub CI, then build, sign, re-download, independently verify, and publish 0.1.3 without running its installer.
