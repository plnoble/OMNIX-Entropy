...
- Status: in progress. Source commit `46374e9` passed local gates and GitHub CI `30333084515`. A 110-file signed payload and independently verified 0.1.1 installer were produced, but the first draft attempt exposed a Windows PowerShell 5.1 native-error handling bug when the expected missing tag returned stderr; no GitHub Release was created. The release lookup is now fixed with an exit-code contract; focused 4/4 and full 1060/1060 tests pass; Release build has zero errors; integrity is 383/18; the script parses; diff check is clean.
- Exact next action: commit and push the release-script fix, wait for replacement CI, then rebuild payload/setup so all manifests and product metadata bind to the final release commit.
