...
- Status: in progress. Release lookup fix commit `6053d7b` passed GitHub CI `30333783060`. Final payload signing then exposed repeated DigiCert timestamp instability; every failed output remained ineligible and was not reused. A GlobalSign R45 probe produced a Valid timestamp, and both signing scripts now allow only that exact additional official HTTP host/path. TDD timestamp contracts are red 2/8 then green 8/8; full 1060/1060, Release 0 errors, integrity 383/18, three script parsers, and diff check pass.
- Exact next action: commit/push the exact timestamp fallback, wait for CI, then rebuild and verify final payload/setup from that commit.
