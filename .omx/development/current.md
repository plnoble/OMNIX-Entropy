...
## Active Release - 2026-07-30 Review and publish 0.1.5
...
- Authorization: the user explicitly requested “审查这批改动，提交并升版本发布”, authorizing review fixes, version metadata/release notes, commit, push, tag/Release creation, and signing with the existing personal publisher. It does not authorize installer execution, product cleanup/uninstall/migration, certificate creation/export, trust-store mutation, antivirus changes, or `LocalMachine` changes.
...
- Status: source review and local 0.1.5 pre-commit gates complete. One beginner-identity defect was corrected; no release-blocking safety or execution-authority finding remains. Full Debug passes 1100/1100; Release builds with 0 warnings/errors and reports ProductVersion 0.1.5/FileVersion 0.1.5.0; source integrity passes 395 files and 18/18 XAML; both Windows PowerShell scripts parse and pass real WPF acceptance with no operation executed; added production mutation-authority matches are zero. Host GitHub authentication and signing prerequisite inspection both pass.
- Exact next action: commit and push the reviewed 0.1.5 source, then require passing GitHub CI on that immutable revision before signing or staging a Release.

## Active Slice - 2026-07-30 System cleanup inventory and app-family decisions
...
- Exact next action: review the combined uncommitted working-tree slice, then separately decide whether to commit/version/publish it. Public and installed 0.1.4 do not contain these changes.
