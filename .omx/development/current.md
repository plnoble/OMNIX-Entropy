...
- Dependencies: health/root-cause evidence, growth snapshots, exact application-family identity, install footprint classification, migration closure state, recommendation cards, and existing local safety pipelines.
- Risks: inventing a cause from incomplete evidence, summing overlapping sizes, choosing a family-level uninstall target, promising cache redirection without application support, or letting conversational text become execution authority.
- Impact scope: local Agent intent/decision composition, beginner presentation, exact navigation handoffs, tests, and WPF acceptance. No cloud dependency and no new direct mutation authority.
- Acceptance: each question produces one plain conclusion, evidence/unknowns, a prioritized next step, and only an allowlisted navigation or auditable local plan; insufficient evidence says so; every real operation still requires the existing confirmation and rollback path.
- Status: complete in the working tree. `AgentDecisionContextBuilder` converts root-cause cards, drive targets, and growth findings into path-free numeric evidence. Computer Agent now answers why C remains full, who grew most in the available window, why D-drive programs can keep C-drive cache, which exact duplicate-family record can enter official uninstall review, and how much of a requested release target is currently covered by low-risk reversible evidence.
...
- System cleanup: old installer candidates, Chromium profile caches, developer caches, Windows diagnostics, per-location age evidence, nested-path deduplication, incomplete lower bounds, and Windows-versus-OMNIX handling are implemented. Mixed unfiltered/age-filtered groups retain only per-location reviewable bytes through `HasAgeFilteredLocations`; the first-view smoke no longer scrolls the Agent conclusion into view.
- Clean uninstall: exact registered-entry authority, official uninstaller, preflight/recovery evidence, post-scan risk groups, low-risk quarantine/restore, and duplicate-family explanation were audited. Family grouping remains explanation-only.
- Install and migration: MSI/Inno/NSIS/Burn/unknown inspection, D-first routing memory, guarded launch, post-install content classification, migration scoring, destination-space/snapshot/rollback gates, and original-C write monitoring are covered. Moving a program never implies its cache or model moved.
- Verification: system-cleanup/drive/product focused 194/194; install/migration focused 225/225; real home WPF smoke passed with `initialSystemCleanupVisible=true`, `noOperationExecuted=true`, zero quarantine manifests, and `.omx/qa-system-cleanup-handling.png`. Independent re-review reported no blocking findings. Existing install/migration GUI screenshots remain visually valid and both smoke scripts parse; a fresh host-permission GUI rerun was unavailable because the platform approval quota was exhausted, so it remains a final unified gate.
- Delivery boundary: the combined Phase 1-9 worktree remains uncommitted and unreleased; public/product version remains 0.1.5.

## Completed Slice - 2026-08-02 Original Phase 7-9 capability closure audit

- Objective: compare the implemented system-cleanup, clean-uninstall, install-control, and migration workflows against the original approved user scenarios, then close only the missing beginner decisions instead of rebuilding already verified safety infrastructure.
- Dependencies: known-cleanup-store inventory, drive root-cause presentation, exact application-family identity, official uninstall/post-scan/quarantine flow, installer detection/routing/post-install report, migration scoring/execution/closure monitoring, and Agent conversation handoffs.
- Risks: calling read-only size evidence "junk", aggregating overlapping stores, treating Downloads or user data as disposable, allowing a family-level action to target several versions, promising that moving a program also moves its cache, or weakening existing confirmation/rollback boundaries to make a button look complete.
- Impact scope: capability matrix, missing read-only evidence, beginner presentation, Agent decision composition, focused tests, and real GUI proof. Existing typed mutation paths remain unchanged unless a separately reviewed gap requires them. No version, commit, push, package, signing, installer execution, installed-product change, or release yet.
- Final audit result: system cleanup, exact clean uninstall, D-first install control, and migration closure all reuse the existing guarded workflows and have focused scenario coverage. Browser/developer/installer cleanup categories, per-location age rules, post-install content classification, exact family identity, recovery evidence, and original-C write monitoring are included in the Phase 9 completion evidence above.
- Acceptance: every original Phase 7-9 scenario is marked implemented with evidence, closed by a focused change, or explicitly deferred with a beginner-safe reason; system-cleanup cards state whether OMNIX or Windows should handle them; duplicate applications never gain family-level execution; moving a program never implies its cache moved without evidence.
- Status: complete; the capability gaps and scenario proofs are summarized in the Phase 9 completion slice above.
- Exact next action: implement Phase 10 decision workflows, then run the unified release gate.

## Completed Slice - 2026-08-02 Beginner symptom triage and safe next steps

- Objective: turn network, sound, Bluetooth/peripheral, display, driver/device, and blue-screen/restart reports into useful beginner conclusions without inventing root cause or granting mutation authority.
- Status: complete in the working tree. The Agent page now exposes six concise first-view choices before free-form input. Symptom reports show what was checked, what remains unknown, urgency, and exactly one allowlisted next step; explicit requests to open a specific Windows page/tool retain the protected shortcut route.
- Safety result: quick choices and free-form triage only use local rule presentation. They cannot run operations, scripts, registry/service/process changes, installers, settings changes, or cloud AI. Negation, clause locality, informational Bluetooth questions, and unrelated setting/open words are covered so the Agent does not manufacture a problem.
- Verification: focused diagnosis suite passes 81/81; full Debug passes 1181/1181; Release builds with 0 warnings/errors; integrity passes 425 files and 19/19 XAML; both PowerShell scripts parse; diff check has no errors beyond existing line-ending warnings. Real WPF smokes prove all six choices in the first working area, checked/unknown/urgency, exactly one next step, protected explicit-tool confirmation, cancellation, no external tool launch, no operation, and zero quarantine manifests.
- Visual evidence: `.omx/qa-agent-symptom-triage.png`, `.omx/qa-agent-troubleshooting-routing.png`, and `.omx/qa-agent-tool-open-confirmation.png`.
- Delivery boundary: combined Phase 1+ changes remain uncommitted; product/public/installed version 0.1.5, installer, certificate, and GitHub Release are unchanged.

## Completed Slice - 2026-08-02 Phase 7 exact-file preview, quarantine, timeline, and restore

- Objective: turn only Phase 6 eligible community-rule files into a stale-safe, user-confirmed, reversible local operation.
- Status: complete in the working tree. A separate `CommunityRuleCacheCleanupPlanBuilder` binds the verified active-pack SHA, exact software registration identity, complete deduplicated file metadata/lineage, count, bytes, and skipped evidence. It refuses changed packs/profiles/processes/files, conflicting overlaps, partial sets, and batches over 64. Before and after the confirmation window the app reloads and hash-verifies the active pack, rescans profiles, resolves exactly one bound entry, and revalidates the full set. `PrepareForConfirmation` then binds current filesystem identity; the specialized handler rechecks pack/profile evidence before delegating to the existing compensated quarantine/timeline/restore handler.
- UX result: trusted cache directories retain their existing flow. Rule-only eligible evidence now enables one `旧缓存文件安全预演`, shows exact file count/space plus kept recent/unknown files without exposing paths, and labels the final dialog as exact old cache files. The drawer scrolls the primary action into the visible work area; cancellation leaves all fixture files and quarantine manifests untouched.
- Verification: focused cache/quarantine suite passes 51/51; full Debug passes 1155/1155; Release builds with 0 warnings/errors; integrity passes 423 files and 19/19 XAML; diff check has no errors; direct-authority and PowerShell parse counts are zero. Temp-directory integration proves two-file quarantine, one timeline entry, and restore. Real WPF smoke proves eligible first-view preview, rule/app/file rechecks, visible final confirmation, recent-file retention, cancellation, zero operation, and nonblank screenshots.
- Visual evidence: `.omx/qa-community-cache-preview.png` and `.omx/qa-community-cache-confirmation.png`.
- Delivery boundary: Phases 1-7 remain uncommitted together; no real third-party rule pack or user file was processed; product/public version 0.1.5, installer, certificate, installed machine, and GitHub Release are unchanged.
- Exact next action: begin Phase 8 with a capability-gap audit of the existing Agent and system-diagnosis surfaces before adding any new user-facing control.

## Completed Slice - 2026-08-02 Phase 6 exclusions and safe-candidate promotion

- Objective: classify community-rule observations as preview-only, refused, or eligible for a later safe preview through an independent OMNIX policy.
- Status: complete in the working tree. The bounded resolver retains an all-or-nothing exact file set only when every matched file fits the candidate cap; partial sets are cleared. The policy refuses system/install/outside/reparse/unsafe/duplicate paths, keeps running/warned/incomplete/registry/remove-self/missing/recent/unknown-type evidence read-only, and promotes only old low-risk file types below recognized cache directories inside Local/Roaming AppData. Managed ignored rules are removed before evaluation.
- UX result: app drawer and rule center now say whether Agent advanced or stopped and include the first refusal reason in beginner language. Exact candidate paths remain absent from ordinary UI; cache cleanup stays disabled until Phase 7 supplies a safe preview and quarantine plan.
- Verification: focused Phase 6 contracts pass 27/27; full Debug passes 1146/1146; Release builds with 0 warnings/errors; integrity passes 420 files and 19/19 XAML; diff check has no errors; Phase 6 authority, beginner-path-leak, bundled-data, and PowerShell parse counts are zero. Real app-drawer and rule-center smokes show the path-boundary refusal, first-view Agent reason, preference round-trip, disabled cleanup, and `OperationExecuted=false`.
- Visual evidence: `.omx/qa-community-cache-conclusion.png`, `.omx/qa-rule-center-status.png`, and `.omx/qa-rule-center-preview.png`.
- Delivery boundary: Phases 1-6 remain uncommitted together; product version/public 0.1.5, installer, installed machine, certificate, and GitHub Release are unchanged.
- Exact next action: implement Phase 7 by converting only current eligible exact files into a stale-safe, identity-bound, reversible quarantine preview through the existing local safety pipeline.

## Completed Slice - 2026-08-02 Phase 5 community rule center and read-only Rule Lab

- Objective: expose optional community-rule status, consent, rollback, conservative preview, and user exclusions through one beginner-contained application workflow.
- Status: complete in the working tree. The application page opens one modal with `Status`, `Read-only preview`, and `Import / update` tabs. Status shows source, license, version, rollback availability, and the no-cleanup boundary. Import/download requires exact descriptor, hash, license, and two explicit confirmations before local activation or HTTPS transport. Rollback is expected-hash guarded. Preview ranks conservative largest observations, keeps raw paths behind technical detail, and supports ignore/restore through an atomic OMNIX-managed preference file.
- Safety result: no community data is bundled or silently discovered; rule findings still cannot create an operation, delete files, modify registry, or enable cache cleanup. Ignore/restore changes only the isolated OMNIX preference store.
- Verification: focused catalog/preference contracts pass 10/10; full Debug passes 1136/1136; Release builds with 0 warnings/errors; source integrity passes 418 files and 19/19 XAML; diff check has no errors; authority, resolver-mutation, bundled-rule-data, and PowerShell parse counts are zero. Isolated WPF acceptance proves visible provenance/safety copy, ranked preview, ignore/restore round-trip, and `OperationExecuted=false`.
- Visual evidence: `.omx/qa-rule-center-status.png` and `.omx/qa-rule-center-preview.png`.
- Delivery boundary: Phases 1-5 remain uncommitted together; product version/public 0.1.5, installer, installed machine, certificate, and GitHub Release are unchanged.
- Exact next action: implement Phase 6 as a pure fail-closed candidate policy before Phase 7 can construct any cleanup preview or quarantine plan.

## Completed Slice - 2026-08-01 Phase 4 software-profile and beginner Agent integration

- Objective: turn an already validated active rule catalog into bounded per-application cache evidence and plain-language app tile/drawer/Agent conclusions.
- Status: complete in the working tree. Active validated catalogs now enrich software profiles on a bounded background scan; evidence is separate from trusted cache paths, survives growth enrichment, retains source/version/hash/warning/lower-bound/sample details, and cannot authorize execution. Tiles show `发现缓存`; the drawer reports a conservative largest observation, stale size, and an Agent `先预览` conclusion while cache cleanup remains disabled for rule-only evidence.
- UX result: Agent advice is first in the drawer; single applications hide irrelevant family explanations; duplicate families still retain exact-entry context. Beginner copy has no raw paths or hashes, while bounded technical detail preserves them.
- Verification: focused enrichment/presentation/ordering suites pass; full Debug passes 1125/1125; Release builds with 0 warnings/errors; source integrity passes 411 files and 18/18 XAML; diff check and `AuthorityHits=0` pass. Isolated WPF smoke confirms both Agent advice and the cache conclusion are fully inside the screen working area, cache cleanup is disabled, and no operation ran. Screenshot: `.omx/qa-community-cache-conclusion.png`.
- Delivery boundary: Phases 1-4 remain uncommitted together; product version/public 0.1.5, installer, installed machine, certificate, and GitHub Release are unchanged.
- Exact next action: implement Phase 5 as an explicit rule center and read-only Rule Lab; do not add another crowded primary navigation item or enable rule-derived cleanup.

## Completed Slice - 2026-08-01 Phase 3 optional community rule-pack management

- Objective: let a user explicitly import or update a community Winapp2-compatible pack without bundling it, silently activating it, or weakening the read-only evidence boundary.
- Dependencies: Phase 1 strict loader, Phase 2 resolver, `AppStoragePathResolver`, immutable content-addressed files, bounded JSON state, write-through staging, atomic pointer replacement, and explicit source/license/hash consent.
- Risks: activating malformed or changed bytes, trusting metadata the user did not review, losing the known-good pack during replacement, path escape/reparse writes, partial state after cancellation, unbounded downloads, or treating a data update as cleanup authority.
- Impact scope: OMNIX-managed per-user rule-pack storage and tests first; a separately bounded download transport may follow only with an already reviewed/pinned descriptor. No third-party data is committed or bundled; no cleanup/UI/version/release action.
- Acceptance: validation and full parse occur before activation; content is addressed by SHA-256; active state changes atomically; one previous descriptor remains rollback-ready; stale confirmation is rejected; corrupt/missing packs fail closed; cancellation leaves active state unchanged; receipts remain non-executable.
- Status: complete in the working tree. User-imported or explicitly downloaded packs are bounded, fully parsed, hash-pinned, stored as immutable content-addressed files, and activated through one atomically replaced state pointer. Source/license/version/hash consent is descriptor-bound; HTTPS downloads have no discovery/background path; one prior descriptor remains stale-safe rollback-ready; corruption and cancellation fail closed.
- Verification: focused Winapp2/AppIdentity contracts pass 27/27; full Debug passes 1119/1119; Release builds with 0 warnings/errors; source integrity passes 407 files and 18/18 XAML; changed-file whitespace/diff checks pass; maintenance/registry/process/background authority hits are zero.
- Exact next action: begin Phase 4 by enriching existing `SoftwareProfile` results with bounded rule evidence, then convert that evidence into beginner Agent conclusions and app-drawer summaries without exposing raw paths on the first surface.

## Completed Slice - 2026-08-01 Phase 2 bounded Winapp2 evidence resolution

- Objective: resolve already attributed Winapp2 file expressions into bounded, cancellable, read-only file-count, size, age, exclusion, and confidence evidence.
- Dependencies: Phase 1 catalog/attribution, `Css.Scanner`, Winapp2 `path|patterns|RECURSE/REMOVESELF` semantics, rule exclusions, reparse-point handling, progress reporting, and temp-directory fixtures.
- Risks: traversing a broad owner root, following links, double-counting overlapping expressions, ignoring exclusions, treating truncated size as exact, retaining too many private paths, or allowing resolved evidence to become an operation.
- Impact scope: shared pure file-expression semantics in `Css.Rules`, a read-only resolver/filesystem adapter in `Css.Scanner`, models, and focused tests. No database bundling/update, UI, cleanup, quarantine, registry access, process launch, version, commit, push, or release.
- Acceptance: only the deepest proven profile root is traversed; roots and returned entries are canonical and contained; reparse points are skipped; files/directories/matches/samples/time are bounded; cancellation propagates; semicolon patterns, recursion, exclusions, deduplication, age, errors, and lower bounds are honest; progress is observable; every result remains non-executable.
- Status: complete in the working tree. The resolver assigns each expression to its deepest proven profile root, prunes unrelated directory branches, skips reparse points, rejects escaped entries and volume roots, applies FILE/PATH exclusions, deduplicates overlaps, and reports age/size/count/progress with honest lower bounds. Every result remains non-executable.
- Verification: focused Winapp2 contracts pass 11/11; full Debug passes 1111/1111; Release builds with 0 warnings/errors; source integrity passes 403 files and 18/18 XAML; changed-file whitespace and diff checks pass; mutation/network/process/registry/operation authority hits are zero.
- Exact next action: begin Phase 3 community rule-pack management with explicit user enablement, pinned metadata/hash, strict pre-activation parsing, atomic replacement, and previous-version rollback. Do not bundle the CC BY-SA database or update it silently.

## Completed Slice - 2026-08-01 Read-only Winapp2 evidence catalog

- Objective: implement the first FluentCleaner-inspired improvement as an independently written, bounded Winapp2-compatible parser and application-attribution layer that expands diagnosis without granting cleanup authority.
- Dependencies: `Css.Rules`, `SoftwareProfile`, deterministic SHA-256 and UTF-8 handling, local fixture tests, and the existing separation between recommendations and `OperationDescriptor` execution.
- Risks: treating a community rule as proof of safe deletion, misattributing similarly named products, accepting changed or malformed rule packs, importing registry mutation authority, copying third-party code/data, or accidentally bundling CC BY-SA data.
- Impact scope: new `Css.Rules` models/services and focused tests only. No bundled Winapp2 database, network updater, UI, scanner wiring, operation descriptor, cleanup, registry access, process execution, version bump, installer, commit, push, or release.
- Acceptance: metadata requires source/version/license/expected hash; pack size, line length, entry and target counts are bounded; malformed/hash-mismatched input fails closed; warnings/defaults/raw evidence are preserved; software attribution requires path evidence; every match is explicitly non-executable; source contracts prohibit mutation/process/operation references.
- Status: complete in the working tree. Added independently written metadata/models, bounded strict-UTF-8 parser, SHA-256 pinning, diagnostics/raw evidence, and literal-anchored wildcard path attribution. Focused contracts pass 6/6; the downloaded Winapp2 260730 compatibility probe parsed 3,721 sections, 11,110 file targets, 3,368 registry targets, and 17 warnings with zero unknown-key diagnostics and `ExecutionAuthorized=False`; full Debug passes 1106/1106; Release builds with 0 warnings/errors; source integrity passes 399 files and 18/18 XAML; changed-file whitespace, diff, and zero-authority scans pass.
- Exact next action: review this uncommitted backend slice, then wire read-only evidence into software-profile enrichment and Agent presentation without bundling a third-party database or attaching an operation.

## Analysis Complete - 2026-08-01 FluentCleaner reference review

- Objective: inspect `builtbybel/FluentCleaner` source and product behavior, then identify ideas that improve OMNIX-Entropy without weakening its beginner-facing safety model.
- Evidence: reviewed shallow clone commit `be347511ae8c639bdc9ca1cfc38dec02fe92c7c5`, the MIT license, Winapp2 parser/path/detection code, analyze/clean engine, Rule Lab, AI/custom-script paths, database updates, automation, UI, CI, and open false-positive reports. No FluentCleaner code or data was copied into this repository.
- Conclusion: the strongest opportunity is an isolated, read-only Winapp2-compatible evidence provider that expands application-cache discovery, preserves source/version/hash/warnings, and feeds existing `SoftwareProfile`, Agent recommendations, dry-run presentation, `OperationPipeline`, quarantine, rollback, and timeline boundaries. Result ranking by recoverable size, scan progress/cancel, global exclusions, and raw-rule technical detail are also useful.
- Rejected behavior: direct `File.Delete` or registry deletion, AI-generated executable PowerShell, unsigned live rule replacement, silent `/AUTO` cleanup, AppX removal by broad matching, and a beginner UI made of thousands of technical checkboxes.
- License boundary: FluentCleaner code is MIT, but the bundled/upstream Winapp2 rule data is CC BY-SA 4.0. OMNIX has no repository license file. Do not bundle or redistribute the database until product licensing and attribution/share-alike boundaries are explicitly decided; prefer an independently implemented parser plus separately managed user-imported or runtime rule packs.
- Status: analysis and comparison complete. Product source, version, installer, installed system, and release state are unchanged.
- Exact next action: if the user approves implementation, start with a read-only `Winapp2RuleCatalog` import/validation slice and user-shaped contracts. Imported rules remain untrusted evidence and must not carry execution authority.

## Completed Release - 2026-08-01 Public 0.1.5

- Objective: review the combined actionable C-drive, impact-aware priority, system-cleanup inventory, and app-family decision worktree; fix release blockers; set version 0.1.5; commit and push `main`; require passing CI; build, sign, independently verify, and publish the guarded GitHub Release.
- Authorization: the user explicitly requested review, commit, version bump, and publication, then explicitly approved host-context read-only Authenticode verification, signing with the existing non-exportable OMNIX certificate, and GitHub Release creation/verification/publication. Installer execution, product cleanup/uninstall/migration, certificate export/creation, trust-store mutation, antivirus changes, and `LocalMachine` changes remained unauthorized.
- Dependencies: cleanly understood combined diff, full unit/Release/integrity/GUI gates, GitHub CLI authentication, GitHub Actions, existing release scripts, Inno Setup, SignTool, existing eligible signer `5688958FEA0056861558E8DCF9D2381AF46074B2`, and release download-back verification.
- Risks: publishing read-only candidates as executable cleanup, family-level uninstall identity confusion, stale version/channel metadata, packaging a revision different from the tag, transport timeouts leaving partial remote state, or calling a draft/update available before public latest verification.
- Impact scope: review and necessary fixes; source version/release notes; repository records; commit/push/CI; signed artifacts and public GitHub Release 0.1.5. No installer launch or installed-machine operation.
- Acceptance: no unresolved release-blocking review findings; exact-entry operation boundaries remain intact; full Debug/Release/integrity/GUI gates pass; one immutable pushed revision binds CI/package/tag/Release; signed setup passes independent verifier; draft assets download back with matching hashes; public latest reports stable 0.1.5.
- Status: complete. Reviewed source commit `234bcd8f412d16f042079e241f158e99342c6121` is pushed and GitHub CI run `30528588089` passed. The target-bound App/worker payload and Inno setup were signed with personal publisher `5688958FEA0056861558E8DCF9D2381AF46074B2` and a GlobalSign timestamp. Independent host-context verification passed for 110 payload files and for the setup policy. A draft with exactly four assets was downloaded back; every length and SHA-256 matched, and the downloaded setup independently returned `CanStageGitHubRelease=true`. Public `releases/latest` now reports non-draft/non-prerelease `v0.1.5`, target `234bcd8`, and the public update manifest returned HTTP 200 with the expected version, commit, length, hash, and signer fields.
- Public artifact: `https://github.com/plnoble/OMNIX-Entropy/releases/tag/v0.1.5`; setup length 14,861,288 bytes; SHA-256 `B49B67F4D3922E81EAD8C80C9F3FBFC8A58EA340B6907975C51EB5C513732BEB`; signer `5688958FEA0056861558E8DCF9D2381AF46074B2`.
- Blocker: none for publication. Disposable-machine behavioral acceptance remains absent, personal signing does not provide public SmartScreen reputation, and no installer/product operation was run during release work.
- Exact next action: the user may open the installed app's update window, check for 0.1.5, review the visible download/install confirmation, and report the resulting installed version. Do not automate or silently run the installer.

## Completed Slice - 2026-07-30 System cleanup inventory and app-family decisions

- Objective: make the product identify material known cleanup stores, explain same-family application records, identify the exact uninstallable entry, and quantify whether moving a main program to D materially changes C-drive use.
- Authorization: the user explicitly said “继续开发” after reviewing the read-only Antigravity/OpenCode and system-cleanup evidence.
- Dependencies: `DriveScanResult`, C-drive root-cause presentation, software registry inventory, `SoftwareProfile`, app catalog tiles/drawer, migration presentation, UIAutomation smoke, and existing exact-entry uninstall/migration safety gates.
- Risks: calling active temp files garbage, manually deleting Windows-managed stores, double-counting nested data paths, over-grouping unrelated products, merging sibling operation identities, or implying that an updater payload is an installed application.
- Impact scope: read-only cleanup opportunity model/probe/presentation; software version/source/data-size evidence; family relationship presentation; quantified migration outcome; tests/UI/records. No cleanup, uninstall, migration, registry, service, process, installer, updater, certificate, version, commit, push, or release execution.
- Acceptance: known cleanup stores have an explicit handling class; inaccessible/truncated sizes are honest; Antigravity/OpenCode fixtures explain family counts and entry roles; only exact profiles with official uninstall commands offer uninstall; the drawer states main-program savings, C data remaining, and expected long-term outcome; all actions retain existing confirmation/snapshot/rollback boundaries.
- Status: complete and included in public 0.1.5 source target `234bcd8`. The system-drive scan inventories bounded known cleanup stores with user/Windows handling classes and honest lower-bound copy. Home and C-drive views separate immediately safe cleanup from larger temp/system-cache candidates. Software profiles preserve exact version/source and bounded data size. Same-family cards explain relationships while keeping official-uninstall authority exact-entry-only, and the drawer quantifies current-program savings, other C-drive family payloads, C-drive data that remains, and expected future growth. Focused contracts pass 12/12; final full Debug passes 1100/1100; Release builds with 0 warnings/errors; source integrity passes 395 files and 18/18 XAML; both PowerShell GUI scripts parse; isolated app-family and home/C-drive WPF smokes pass with no operation executed.
- Visual evidence: `.omx/qa-app-family-decision.png`, `.omx/qa-home-agent-next-action.png`, `.omx/qa-drive-health-opportunities.png`, and `.omx/qa-drive-health-plan.png`.
- Exact next action: obtain a user-run 0.1.5 in-app update receipt; keep cleanup and family operations behind their existing evidence, confirmation, and rollback gates.

## Analysis Complete - 2026-07-30 System cleanup and duplicate application clarity

- Objective: understand why the product misses material system-cleanup candidates, shows Antigravity/OpenCode as unexplained duplicate cards, and cannot answer whether moving a main program to D prevents future C-drive growth.
- User evidence: screenshots show two Antigravity cards and three OpenCode cards with only red-dot C-drive labels; the user cannot tell which entry is an installed version, data/updater evidence, or a safe uninstall target.
- Read-only machine evidence: the first 20,000 files under the current-user temp directory total at least about 4.59 GB; crash dumps are about 102.2 MB, DirectX shader cache about 13.4 MB, and Windows Update download cache about 5.2 MB. Current root-oriented recommendations expose `C:\tmp` but do not promote these nested Windows/user cleanup stores.
- Application evidence: Antigravity has about 1.21 GB under `D:\Agent\Google AntiGravity`, plus about 263.5 MB updater data and 88.7 MB roaming data on C. OpenCode has a registered 1.14.41 installation under `C:\Program Files` at about 446.7 MB, separate D locations totaling about 719 MB, an updater payload around 237.3 MB, and about 88.3 MB roaming data on C. Current registry evidence exposes one official OpenCode 1.14.41 uninstall target and no current Antigravity uninstall entry.
- Root causes: system cleanup detection is incomplete below the drive root; software inventory dedupes only exact display-name/install-path pairs; profiles do not preserve display version/source identity; data size is not populated; application tiles do not explain family relationships; migration copy does not quantify main-program savings versus C-drive data that remains.
- Safety constraints: cleanup candidates are not automatically garbage; Windows-managed stores must use Windows cleanup mechanisms; unregistered D copies are not deletion targets; duplicate-family grouping must not merge operation identities; uninstall remains exact-entry and official-uninstaller only.
- Recommended next slice: add read-only system-cleanup opportunity classification; preserve version and inventory source; calculate bounded user-data size; add a family relationship layer that keeps operations entry-specific; show “main program can save / C data remains / long-term result” in the drawer. Add execution only after separate evidence and consent.
- Status: analysis complete; no product source changed in this investigation. The prior impact-aware C-drive working tree remains uncommitted and public/installed 0.1.4 remains unchanged.

## Completed Slice - 2026-07-30 Impact-aware C-drive priorities

- Objective: correct the beginner-facing C-drive plan when a safe cleanup is real but too small to materially improve drive health.
- User evidence: the current scan needs about 19.5 GB to return to the 80% comfort target, while the immediately safe cleanup is only 337.6 MB, about 1.7% of that gap.
- Dependencies: `DriveHealthPlanPresenter`, bounded key-finding ordering, homepage/detailed-plan actions, UIAutomation smoke, and existing low-risk recommendation contracts.
- Risks: hiding a safe option, presenting observation-only sources as deletable, inventing a guaranteed large recovery amount, or making an arbitrary score more important than actual free space.
- Impact scope: read-only priority, quantified beginner copy, key-finding ordering, focused tests, GUI acceptance, and records. No scanner authority, operation descriptor, cleanup eligibility, quarantine, installer, updater, certificate, or release change.
- Acceptance: safety determines whether an action may be offered, while material impact determines what the Agent recommends first; a safe cleanup below 10% of the target becomes an optional secondary action when larger read-only evidence exists; the UI states its percentage contribution and remaining gap; a bounded key-finding list retains one safe item without ranking it above larger sources; no operation executes during acceptance.
- Status: complete in the working tree. Safe cleanup is now an eligibility class rather than an automatic first priority. A contribution below 10% becomes an optional secondary action when larger evidence exists; the primary action is read-only investigation of personal files or major sources. The plan states contribution percentage and remaining gap, while key findings rank impact first but retain one low-risk item. Focused tests pass 12/12; full Debug passes 1087/1087; Release builds with 0 warnings/errors; source integrity passes 390 files and 18/18 XAML; the real WPF smoke verifies impact-first navigation, optional safe preview, and `noOperationExecuted=true`.
- Visual evidence: `.omx/qa-home-agent-next-action.png`, `.omx/qa-drive-health-opportunities.png`, and `.omx/qa-drive-health-plan.png`.
- Exact next action: review and commit the combined actionable/impact-aware working-tree slice, then decide separately whether to version and publish it. Public/installed 0.1.4 is unchanged.

## Completed Slice - 2026-07-30 Actionable C-drive conclusion

- Objective: fix the post-scan experience where real C-drive opportunities are present but the beginner-facing conclusion feels like there is nothing to adjust.
- User evidence: the latest local digest at 2026-07-30 14:06 reports C at 86.5%, score 64, about 337.6 MB potentially safe to process, five unexpected-root findings, and 105 ordinary C-drive application clues, yet also reports zero low-risk cleanup findings.
- Dependencies: recommendation ordering, health-digest counts, `DriveHealthPlanPresenter`, C-drive plan buttons, root-cause navigation, and the existing confirmation/quarantine pipeline.
- Risks: turning an observation into deletion authority, overstating 337.6 MB as enough to solve the 80% gap, adding another dense report, or providing a button that only moves focus without a clear next decision.
- Impact scope: beginner conclusion ordering/copy, one read-only secondary navigation action, focused tests, GUI evidence, and development records. No scanner authority, cleanup eligibility, operation descriptor, quarantine behavior, installer, updater, certificate, or trust behavior changes.
- Acceptance: executable low-risk recommendations cannot be hidden behind observation-only findings; the plan says how many items can be handled now and how many still need investigation; it states the remaining comfort gap honestly; when safe cleanup is insufficient, a visible secondary action leads to the next read-only space source; no operation runs in automated acceptance.
- Status: complete in the working tree. Low-risk cleanup recommendations now outrank observation-only cards in the five key findings; the plan says “不是没有可调整项”, counts immediately safe and investigation items separately, preserves the remaining 80% gap, and exposes a second read-only “继续找更大的占用/文件” action. The existing cleanup eligibility, confirmation, quarantine, and operation pipeline are unchanged. Focused tests pass 11/11 plus 226/226 adjacent experience tests; full Debug passes 1086/1086; Release builds with 0 warnings/errors; source integrity passes 390 files and 18/18 XAML; mutation-authority search is zero; real WPF smoke shows both actions, correct quantified copy, and `noOperationExecuted=true`.
- Visual evidence: `.omx/qa-home-agent-next-action.png`, `.omx/qa-drive-health-plan.png`, and `.omx/qa-drive-health-opportunities.png`.
- Exact next action: review and commit this source slice, then decide separately whether to version and publish it. Public/installed 0.1.4 does not contain the working-tree correction.
