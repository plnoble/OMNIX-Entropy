...
- Status: in progress. Version 0.1.1 and release notes are prepared; focused 183/183 and full 1060/1060 tests pass; Release build succeeds with 18 environmental NU1900 warnings and zero errors; source integrity reports 383 valid UTF-8 files and 18/18 valid XAML files; five release scripts parse; diff check is clean. The restricted tool environment could not see the personal signer, but an approved host-session read-only query confirmed the original private key remains in CurrentUser My and the same public certificate remains in CurrentUser TrustedPeople, TrustedPublisher, and Root. No replacement identity is needed.
- Exact next action: review and commit/push the source revision, then wait for GitHub CI before building the signed payload and installer from that committed revision.
